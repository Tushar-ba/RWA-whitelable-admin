...
export default function BuyTransaction() {
  ...
  const filteredTransactions = transactions.filter((transaction: any) => {
    ...
    const matchesMetal = metalFilter === "all" || transaction.metal === metalFilter || transaction.assetType === metalFilter;
    const matchesNetwork = networkFilter === "all" || transaction.network === networkFilter || transaction.networkType === networkFilter;
    const matchesPaymentMethod = paymentMethodFilter === "all" || transaction.paymentMethod === paymentMethodFilter;
    return matchesSearch && matchesStatus && matchesMetal && matchesNetwork && matchesPaymentMethod;
  });
  ...
  const csvData = filteredTransactions.map((transaction: any) => ({
    ...
    'User': transaction.userName || 'Unknown',
    'Asset Type': transaction.metal || transaction.assetType,
    'Amount': `${transaction.tokenAmount || transaction.amount} ${transaction.metal === 'gold' ? 'oz' : 'oz'}`,
    'USD Amount': `$${transaction.usdAmount || transaction.totalValue}`,
    'Payment Method': transaction.paymentMethod,
    'Status': transaction.status,
    'Network': transaction.networkType || transaction.network || 'ethereum',
    'Date': transaction.date || formatDate(transaction.transactionDate || new Date())
  }));
  ...
  const totalValue = filteredTransactions
    ...
    .reduce((sum: number, t: any) => sum + parseFloat(t.usdAmount || t.totalValue || 0), 0);
  const pendingTransactions = filteredTransactions.filter((t: any) => t.status === "pending").length;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">Buy Transactions</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-600">All buy transactions</p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Buy Transaction History</CardTitle>
            <CSVLink
              data={csvData}
              filename={`buy-transactions-${new Date().toISOString().split('T')[0]}.csv`}
                ...
                <TableRow key={transaction.id || transaction._id}>
                  ...
                  <TableCell className="font-medium">
                    {transaction?.userId?.first_name && transaction?.userId?.last_name 
                      ? `${transaction.userId.first_name} ${transaction.userId.last_name}`
                      : transaction?.userId?.email || transaction.userName || 'Unknown User'}
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      <div className={`h-3 w-3 rounded-full ${
                        (transaction.metal || transaction.assetType) === 'gold' ? 'bg-yellow-500' : 'bg-gray-400'
                      }`}></div>
                      <span className="capitalize">{transaction.metal || transaction.assetType}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    {transaction.tokenAmount || transaction.amount} oz
                  </TableCell>
                  <TableCell>
                    ${(parseFloat(transaction.usdAmount || transaction.totalValue || '0') / parseFloat(transaction.tokenAmount || transaction.amount || '1')).toFixed(2)}
                  </TableCell>
                  <TableCell className="font-semibold">
                    ${transaction.usdAmount || transaction.totalValue || '0'}
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className="bg-green-50 text-green-800">
                  ...
                  <TableCell>
                    {getNetworkBadge(transaction.networkType || transaction.network || "ethereum")}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
